#' Conflicts between the fmsea and other packages
#'
#' This function lists all the conflicts between packages in the fmsea
...
#'
#' @export
#' @return fmsea_conflicts
#' @examples
#' fmsea_conflicts()
fmsea_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", fmsea_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "fmsea_conflicts")
}

fmsea_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "fmsea_conflicts()"
  )
...
#' @export
print.fmsea_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(fmsea_conflict_message(x))
}

...
fmsea_attach <- function() {
  to_load <- fmsea_core_unloaded()
  if (length(to_load) == 0)
    return(invisible())
  
  msg(cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("fmsea ", fmsea_package_version("fmsea"))
  ),
  startup = TRUE)
  
  versions <-
    vapply(to_load, fmsea_package_version, character(1))
  packages <- paste0(
...
fmsea_package_version <- function(x) {
